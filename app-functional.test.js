...
test("both forms set date of test to date of cast plus age in days", () => {
  ...
  for (const [html, url] of forms) {
    ...
    const row = global.document.querySelector(".results-table tbody tr");
    assert.ok(row, `${url} should seed a result row`);
    ...
    assert.ok(castInput, `${url} should have a date of cast input`);
    assert.ok(ageInput, `${url} should have an age input`);
    assert.ok(testInput, `${url} should have a date of test input`);
    assert.equal(ageInput.type, "number", `${url} age field must be a number of days`);
    assert.equal(ageInput.getAttribute("aria-label"), "Row 1 age in days");

    castInput.value = "2026-06-01";
    ageInput.value = "28";
    ageInput.dispatchEvent(new global.Event("input", { bubbles: true }));
    assert.equal(testInput.value, "2026-06-29", `${url} date of test should be cast + 28 days`);

    castInput.value = "2026-12-28";
    castInput.dispatchEvent(new global.Event("change", { bubbles: true }));
    assert.equal(testInput.value, "2027-01-25", `${url} date of test should follow a new cast date`);
    assert.equal(ageInput.value, "28", `${url} should keep the entered age in days`);
