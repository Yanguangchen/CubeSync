...
test("rpa-dashboard.js theme toggle", async () => {
  const dom = new JSDOM(rpaDashboardHtml, {
    runScripts: "dangerously",
    url: "http://localhost/"
  });
  ...
  assert.ok(themeToggle, "Theme toggle should exist");

  // Default is light (checked)
  assert.strictEqual(themeToggle.checked, true);
