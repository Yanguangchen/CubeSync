...
7. Click **Save / Submit**. On success the request is stored as **Draft** for staff review. This browser also keeps a **copy of that submission** (up to 25 copies or about 400 KB; the oldest copy is removed first when the limit is reached). Later, **Previous submissions** lists copies saved on this device (not a lab login). Opening a copy raises a paper preview of the full form; it does not overwrite the form you were filling. **Edit** unlocks that copy; **Close** puts it away. **Forget copies on this device** removes the local list only.
...
- **Previous submissions** — copies of forms already saved from this browser, kept on the device (up to 25 copies or about 400 KB; oldest removed first). Opening one shows a paper preview; **Edit** unlocks it, **Close** returns to the form you were filling. No sign-in. Clearing site data or using another device will not show them.
